...
double t(double z,double r)
{
    return 205.75*pow(e,-0.02852*z-0.1752*r);
}
...
//-----------------------------------------------------------//
    ...
    for(double z=0.0;z<=0.5;z+=0.02)
    {
        for(double r=0.0;r<=0.1;r+=0.01)
        {
            glColor4f(t(z*100,r*100)/100,0,1-t(z*100,r*100)/100,0.1);
            for(int j=0;j<=63;j++)
            {
                glPushMatrix();
                glBegin(GL_LINES);
                glVertex3f(r*sinf(2*pi*j/64),z,r*cosf(2*pi*j/64));
                glVertex3f(r*sinf(2*pi*(j+1>64?1:j+1)/64),z,r*cosf(2*pi*(j+1)/64));
                glEnd();
                glPopMatrix();
            }
