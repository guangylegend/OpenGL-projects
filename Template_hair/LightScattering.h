...
class LightScattering
{
    ...
    const double longshift_R = 3.0*piover180;
    const double longshift_TT = 1.5*piover180;
    const double longshift_TRT = 6*piover180;

    const double longwidth_R = 0.5*piover180;
    const double longwidth_TT = 0.5*piover180;
    const double longwidth_TRT = 25.0*piover180;
    ...
    {
        return (1.0/(sqrt(2*pi)*sdeviation)) * pow(e,-x*x/(2*sdeviation*sdeviation));
    }
...
public:
    ...
    double scattering(double Phi_i, double Phi_r, double Theta_i, double Theta_r, int rgb)
    {
        phi_i = Phi_i;
        phi_r = Phi_r;
        phi = phi_r-phi_i;
